...
It currently only supports time series channels.  Note: The sequence channel V1 API is deprecated and will be supported for a period of time.
...
* Why does /lastwritten return an empty array of uris?
...
## fetch lastwritten channel items


To retrieve the lastwritten items inserted into a channel, issue a HEAD or GET request on the `lastwritten` link returned from the channel
metadata.  The Hub will issue a 303 redirect to the list of items at that time.


`HEAD http://hub/channel/stumptown/lastwritten`

On success:  `HTTP/1.1 303 See Other`
`Location: http://hub/channel/stumptown/2013/04/23/20/42/31`

Here is how you can do this with curl:

`curl -I http://hub/channel/stumptown/lastwritten`
