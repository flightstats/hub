...
public class S3Config {
    ...
    private class S3ConfigLockable implements Lockable {
        ...
        private void updateTtlDays() {
            ...
            if (!rules.isEmpty()) {
                log.info("********************before merging all rules {} ",rules.size());
                rules.forEach(rule->log.info("****************** {}",rule.getId()));

                GetBucketLifecycleConfigurationRequest getBucketLifecycleConfigurationRequest = new GetBucketLifecycleConfigurationRequest(s3Properties.getBucketName());
                List<BucketLifecycleConfiguration.Rule> nonHubBucketLifecycleRules =
                        S3ConfigStrategy.getNonHubBucketLifecycleRules(s3Client.getBucketLifecycleConfiguration(getBucketLifecycleConfigurationRequest));

                log.info("********************nonHubBucketLifecycleRules*******************{} ", nonHubBucketLifecycleRules.size());
                nonHubBucketLifecycleRules.forEach(rule->log.info("****************** {}",rule.getId()));

                rules.addAll(nonHubBucketLifecycleRules);

                log.info("********************after merging all rules {} ",rules.size());
                rules.forEach(rule->log.info("****************** {}",rule.getId()));
                ...
                List<BucketLifecycleConfiguration.Rule> finalRules = s3Client.getBucketLifecycleConfiguration(getBucketLifecycleConfigurationRequest).getRules();
                log.info("********************after updating s3 rules {} ",finalRules .size());
                finalRules.forEach(rule->log.info("****************** {}",rule.getId()));
