...
class WebhookError {
    ...
    @Inject
    public WebhookError(WebhookErrorService webhookErrorService, WebhookErrorReaper webhookErrorReaper, ChannelService channelService) {
        this.webhookErrorService = webhookErrorService;
        this.webhookErrorReaper = webhookErrorReaper;
    ...
    public Set<String> getWebhooks() {
        return webhookErrorService.getWebhooks();
    }
    ...
    private List<com.flightstats.hub.webhook.error.WebhookError> limitChildren(String webhook) {
        ...
        List<com.flightstats.hub.webhook.error.WebhookError> errorsToRemove = webhookErrorReaper.getErrorsToRemove(errors);

        errorsToRemove.forEach(error -> webhookErrorService.delete(webhook, error.getName()));

        return errors.stream()
                .filter(error -> !errorsToRemove.contains(error))
                .collect(toList());
    }
