...
public class SpokeBatchContentDao implements ContentDao {
    ...
    public ContentKey insert(String channelName, Content content) throws Exception {
        ...
        if (!spokeStore.insert(path, content.getData(), "batch", "payload", channelName)) {
    ...
    private SortedSet<ContentKey> queryByTimeKeys(TimeQuery query) {
        try {
            ...
            QueryResult queryResult = spokeStore.readTimeBucket("batch", query.getChannelName(), timePath);
            ActiveTraces.getLocal().add("spoke query result", queryResult);
            if (!queryResult.hadSuccess()) {
                QueryResult retryResult = spokeStore.readTimeBucket("batch", query.getChannelName(), timePath);
    ...
    public void delete(String channelName) {
        try {
            spokeStore.delete("batch", channelName);
        } catch (Exception e) {
