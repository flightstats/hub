...
public class CallbackQueue implements AutoCloseable {
    ...
    //todo - gfm - 12/2/14 - should probably include last delivered ContentKey
            ...
            public void run() {
                ...
                while (!shouldExit.get()) {
                    ...
                    if (lastTime.isBefore(stableOrdering)) {
                        //todo - gfm - 12/3/14 - do we want a convenience method that doens't need this?
                        TimeQuery query = TimeQuery.builder().channelName(channel).startTime(lastTime).unit(TimeUtil.Unit.SECONDS).build();
                        Collection<ContentKey> keys = contentService.queryByTime(query);
