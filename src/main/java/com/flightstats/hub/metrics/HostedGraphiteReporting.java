...
public class HostedGraphiteReporting implements Haltable {
    ...
    private void registerJvmMetrics(MetricRegistry registry) throws UnknownHostException {
        String hostPrefix = InetAddress.getLocalHost().getHostName().split("\\.")[0];
        registry.registerAll(prefix(hostPrefix + "jvm.gc", new GarbageCollectorMetricSet()));
        registry.registerAll(prefix(hostPrefix + "jvm.memory", new MemoryUsageGaugeSet()));
        registry.registerAll(prefix(hostPrefix + "jvm.threads", new ThreadStatesGaugeSet()));
    }
