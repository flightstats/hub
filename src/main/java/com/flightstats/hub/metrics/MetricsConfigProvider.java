...
public class MetricsConfigProvider {
    private final Logger logger = LoggerFactory.getLogger(MetricsConfigProvider.class);
    ...
    @Getter(lazy = true)
    private final String influxdbProtocol = safeGetProperty("metrics.influxdb.protocol", "http");
    ...
    @Getter(lazy = true)
    private final String influxdbDatabaseName = safeGetProperty("metrics.influxdb.database.name", "hub_main");

    MetricsConfigProvider(Properties customProperties) {
